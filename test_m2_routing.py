...
@pytest.mark.anyio
async def test_colorize_uses_m2_when_online(monkeypatch, tmp_path):
    input_path = tmp_path / "input.jpg"
    output_path = tmp_path / "output.jpg"
    input_path.write_bytes(TINY_JPEG)
    ...
    result = await provider.process_photo(str(input_path), str(output_path), True, None)

    assert result.success
    assert "https://remote.example/api/restore" not in [url for url, _body in requests]
    assert ("https://m2.example/api/restore", "colorize") in [
        (url, body.get("task")) for url, body in requests
    ]
